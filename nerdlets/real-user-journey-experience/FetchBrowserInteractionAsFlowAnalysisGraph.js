...
export default class FetchBrowserInteractionAsFlowAnalysisGraph extends React.Component {
  constructor(browserInteraction) {
    ...
    this.INTERACTIONS_QUERY =
      "FROM BrowserInteraction SELECT uniqueCount(session), average(duration) FACET browserInteractionName, domain where appName = '$BR_APP_NAME$' and category IN ('Initial page load','Route change') AND previousUrl != targetUrl AND previousGroupedUrl LIKE '$PREVIOUS_URL$' $TIME_RANGE$";
    ...
    if (
      (this.state.accountId != null &&
        this.state.accountId !=
          newBrowserInteraction.applicationDetails.accountId) ||
      (this.state.appName != null &&
        this.state.appName != newBrowserInteraction.applicationDetails.name) ||
      (this.state.timeRangeClause != null &&
        this.state.timeRangeClause != newBrowserInteraction.timeRangeClause)
    ) {
      ...
      if (this.interactions.length > 1) {
        ...
        this.interactions.forEach(thisInteraction => {
          if (!uniqueNodes.find(node => node.id === thisInteraction.target)) {
            ...
            const interactionsWithThisTarget = this.interactions.filter(
              interaction => interaction.target === thisInteraction.target
            );
            ...
            if (interactionsWithThisTarget.length == 1) {
              const avgDurationItem = {};
              avgDurationItem.text = 'page load time';
              avgDurationItem.value = `${thisInteraction.avgDuration.toFixed(
                3
              )} (s)`;
              nodeDetails.value.items.push(avgDurationItem);
            } else {
              ...
              interactionsWithThisTarget.forEach(interaction => {
                ...
                avgDurationItem.value = `${interaction.avgDuration.toFixed(
                  3
                )} (s)`;
                nodeDetails.value.items.push(avgDurationItem);
              });
        ...
        this.allPathsAvgDuration =
          allPathTotalDuration / this.allInteractionsPaths.length;
      }

      // Update rank sep pixels
      const nodeLen = uniqueNodes.length;
      if (nodeLen == 2) {
    ...
    if (interactionsWithThisSource.length == 0) {
      ...
      interactionsWithThisSource.forEach(interaction => {
        ...
        if (decoratedSrcPath.includes(`~${interaction.target}~`)) {
          ...
          const totalDuration = duration + interaction.avgDuration;
          const stepDisplay = `Step${stepNumber}: ${interaction.source} -> ${interaction.target}`;
  ...
  isValidEdge(
    interactionsArr,
    thisSrcInteractionName,
    thisDestInteractionName
  ) {
    ...
    if (
      interactionsArr.find(
        edge =>
          edge.srcInteractionName === thisSrcInteractionName &&
          edge.targetInteractionName === thisDestInteractionName
      )
    ) {
    ...
    if (
      interactionsArr.find(
        edge =>
          edge.srcInteractionName === thisDestInteractionName &&
          edge.targetInteractionName === thisSrcInteractionName
      )
    ) {
    ...
    if (
      interactionsArr.find(
        edge => edge.srcInteractionName === thisDestInteractionName
      )
    ) {
  ...
  async appendChildren(interactionName, srcDisplayName) {
    ...
    let interactionsQuery = this.INTERACTIONS_QUERY.replace(
      '$BR_APP_NAME$',
      this.state.appName
    );
    interactionsQuery = interactionsQuery.replace(
      '$TIME_RANGE$',
    ...
    if (response && response.data && response.data.facets.length > 0) {
      ...
      response.data.facets.forEach(facetInfo => {
        ...
        const childBrowserInteractionDetail = {};
        ...
        childBrowserInteractionDetail.avgDuration =
          facetInfo.results[1].average;
    ...
    const config = {
      ...
      onReady: graph => {
        ...
        graph.on('node:click', evt => {
          ...
          const nodeBrowserInteractionDetail = {};
          nodeBrowserInteractionDetail.accountId = this.state.accountId;
          nodeBrowserInteractionDetail.appName = this.state.appName;
          nodeBrowserInteractionDetail.browserInteractionName =
            item._cfg.model.interactionName;
